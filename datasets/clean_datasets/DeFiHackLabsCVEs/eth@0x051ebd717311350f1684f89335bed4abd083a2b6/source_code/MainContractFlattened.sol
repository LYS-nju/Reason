...
library DODOMath {
    ...
    function _GeneralIntegrate(
        ...
        uint256 fairAmount = i.mul(V1.sub(V2)); 
        if (k == 0) {
    ...
    function _SolveQuadraticFunctionForTrade(
        ...
        uint256 part2 = k.mul(V0).div(V1).mul(V0).add(i.mul(delta)); 
        uint256 bAbs = DecimalMath.ONE.sub(k).mul(V1); 
        bool bSig;
        if (bAbs >= part2) {
            bAbs = bAbs - part2;
            bSig = false;
        ...
        squareRoot = bAbs.mul(bAbs).add(squareRoot).sqrt(); 
