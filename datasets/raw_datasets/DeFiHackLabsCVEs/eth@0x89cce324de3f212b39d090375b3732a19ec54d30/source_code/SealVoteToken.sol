...
contract SealVoteToken is Ownable {
    using SafeMath for *;

    string public name = "SealVoteToken";
    string public symbol = "SVT";
    uint8 public decimals = 18;

    address[2][] public pairs;
    ERC20 seal = ERC20(0x33c2DA7Fd5B125E629B3950f3c38d7f721D7B30D);

    constructor() public {
        pairs.push([0xf54B26A9ec6251afC5c7a45e63b775b6a07FF7AD, 0x199325C32e7ead20FBC0576CbBFde865214165D1]);
        pairs.push([0x2ABf1CF836B321B8e77Ba1C937b6de2536c4e87e, 0x5d39C4e27d9eaa421ADb37D8860c9ece771671e2]);
        pairs.push([0x4ffcE2b2a325F60219701A57EC5ED379749c4c90, 0xe42260676A8811B74A18412845a3140078434532]);
        pairs.push([0xae590578cc96389FA477d5cC0e461a5BeceddF9e, 0xb91c10B39D1941a3a562F6624d5C9358713049F5]);
        pairs.push([0x82b1506Cc8c184b256cdE4430ae68C2e49a8673C, 0xa01A3A5C2356D3e8e012e02FDD5B0eC21C9E6710]);
    }

    function addPair(address[2] calldata _pair) external onlyOwner {
        pairs.push(_pair);
    }

    function removePair(uint256 index) external onlyOwner {
        if(index < pairs.length-1) {
            pairs[index][0] = pairs[pairs.length-1][0];
            pairs[index][1] = pairs[pairs.length-1][1];
        }
        pairs.pop();
    }

    function totalSupply() external view returns (uint256) {
        return seal.totalSupply();
    }
    ...
    function balanceOf(address owner) external view returns (uint256) {
        ...
        for(uint256 i = 0; i < pairs.length; i++) {
            ...
            IUniswapV2Pair pair = IUniswapV2Pair(pairs[i][0]);
            Farm farm = Farm(pairs[i][1]);

            uint256 a = farm.totalValue();
            uint256 b = farm.totalSupply();
            uint256 c = farm.balanceOf(owner);

            uint256 d = pair.totalSupply();

            uint256 e = seal.balanceOf(pairs[i][0]);

            // user LP staked = a*c/b
            // seal per LP = e/d
            // user seal in staked LP = a*c/b * e/d
            balance = balance.add( a.mul(c).div(b).mul(e).div(d) );
